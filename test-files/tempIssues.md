🔴 High Impact
[Done] 1. #include virtual="..." is only half-implemented
...
[Done] 2. No Option Explicit awareness
...
[Done] 3. COM object tracking is one-level deep only
...
[Done] 4. Formatter has no undo safety
...
[Done] 9. #include depth is only one level
...
tsaspConfig.update('wordBasedSuggestions', 'off', vscode.ConfigurationTarget.Global)
...
tsconst lastOpen  = textBefore.lastIndexOf('<%')
...
tsitem.detail = fromInclude ? `Variable (from ${require('path').basename(v.filePath)})` : ...
...
15. formatter_htmlFormatter.ts — No progress notification for large files
...
json{ "open": "'", "close": "'", "notIn": ["string", "comment"] }
